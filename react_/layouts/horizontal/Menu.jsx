...
const AppMenu = ({ menuItems, currentUser }) => {
  ...
  function filteredRoles(menuItem) {
    return menuItem?.roles?.some((role) => currentUser?.roles?.includes(role));
  }

  useEffect(() => {
    //controlling how many menu items can be displayed in it

    let menuFiltered = menuItems.filter(filteredRoles);
    let modifiedMenuItems = menuFiltered ? menuFiltered.filter((item) => (!item.isTitle ? item : null)) : [];
    const defaultDisplayedItems = window.screen.width > 1200 ? 7 : 5;
...
MenuItemWithChildren.propTypes = {
  ...
  subMenuClassNames: PropTypes.string.isRequired,
  activeMenuItems: PropTypes.arrayOf(PropTypes.string).isRequired,
